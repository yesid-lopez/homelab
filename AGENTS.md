...
Single source of truth for all AI coding agents working in this repo (Claude
Code, OpenCode, Cursor, Codex). `CLAUDE.md` imports this file — edit rules here,
not there.

## Agent Rules

- **Never delete PersistentVolumeClaims (PVCs) or volumes** unless I explicitly
  consent.
- When I ask you to **deploy** something to the cluster, commit the changes and
  push to `main` (Flux reconciles from `main`).
- **Helm values live in a separate file** referenced from the `HelmRelease` via
  `valuesFrom` (a `configmap-helm-values.yaml`). Do not inline complex values in
  the `HelmRelease`.
- All secrets go through **Sealed Secrets**. Never commit plain Kubernetes
  secrets.
...
Use these apiVersions when generating manifests (they match what is deployed):

- **HelmRelease**: `helm.toolkit.fluxcd.io/v2`
- **HelmRepository / GitRepository / OCIRepository**: `source.toolkit.fluxcd.io/v1`
...
│   └── ...                  # Other applications
├── infrastructure/
│   ├── controllers/         # Infrastructure controllers (MetalLB, NGINX, cert-manager, etc.)
...
> Applications live under `apps/production/<app>/`. `infrastructure/controllers/`
> is for cluster infrastructure (MetalLB, NGINX, cert-manager, external-dns, …),
> not application workloads.
...
- `configmap-helm-values.yaml`: Values configuration (referenced via `valuesFrom`)

Keep simple values inline only when trivial; anything non-trivial goes in the
ConfigMap. Pin chart versions for production releases.
...
- **multica**: Multica agent platform (CNPG Postgres + uploads in MinIO S3 bucket `multica-uploads`)
...
scripts/seal-client-basic-auth.sh <client>    # (only if the client needs basic-auth)
...
Do not hand-write client manifests — the templates in
`apps/production/client-sites/_templates/` are the source of truth.

### Conventions

- **Namespace**: all clients share `lulo-demo-landings`. Never create a
  per-client namespace. Per-client resources (deployment, service, ingress) are
...
- **Images must be multi-arch** (`linux/amd64,linux/arm64`). The cluster has a
  ...
  CrashLoopBackOff. Always build with `docker buildx build --platform
